<template>
  ...
  >
    ...
    >
          ...
          <template slot="selection" slot-scope="data">
            <div class="v-select__selection v-select__selection--comma">
              ...
              <span v-if="![null, undefined].includes(data.item[itemTitle])">{{
                data.item[itemTitle] +
                  (fullSchema.type === "array" &&
                    data.index !== modelWrapper[modelKey].length - 1
                    ? ",&nbsp;"
                    : "")
              }}</span>
            </div>
      ...
      <v-select
        v-else-if="
          fullSchema['x-display'] !== 'list' &&
            (fromUrl || fullSchema['x-fromData'])
        "
        v-model="modelWrapper[modelKey]"
        :items="selectItems"
        :name="fullKey"
        ...
        :item-value="itemKey"
        :return-object="
          (fullSchema.type === 'array' && fullSchema.items.type === 'object') ||
      ...
      <editor
        v-else-if="
          fullSchema.type === 'string' &&
            fullSchema['x-display'] !== undefined &&
            fullSchema['x-display'].startsWith('editor')
        "
        v-model="modelWrapper[modelKey]"
        :name="fullKey"
        :label="label"
        ...
        :required="required"
        :rules="rules"
        :language="fullSchema['x-display'].split('-')[1]"
        filled
        class="v-text-field--box v-text-field--enclosed"
        @change="change"
        @input="input"
          ...
          <v-layout v-show="!foldable || !folded" row wrap class="ma-0">
            ...
            <template v-if="fullSchema.allOf && fullSchema.allOf.length">
              <template v-if="!parentKey && fullSchema.allOf[0].title">
                ...
                >
                  <v-tabs-slider />
                  <v-tab
                    v-for="(currentAllOf, i) in fullSchema.allOf"
                    :key="i"
                    :href="`#tab-${i}`"
                  >
                    {{ currentAllOf.title }}
            ...
            <template v-if="fullSchema.oneOf">
              <v-select
                ...
                :item-value="
                  item => {
                    return oneOfConstProp
                      ? item.properties[oneOfConstProp.key].const
                      : item.title;
                  }
                "
                :label="
                  oneOfConstProp
                    ? oneOfConstProp.title || oneOfConstProp.key
                    : 'Type'
                "
              ...
              >
                <tooltip
                  ...
                  :options="options"
                  :html-description="
                    oneOfConstProp && oneOfConstProp.htmlDescription
                  "
                />
              </v-select>
              <!--{{ currentOneOf }}-->
...
export default {
  name: 'Property',
  components: { SelectIcon, SelectItem, Tooltip, Editor },
  props: [
    'schema',
    'modelWrapper',
    'modelRoot',
  ...
  computed: {
    ...
    fromUrlKeys() {
      ...
      return matchAll(this.fullSchema['x-fromUrl'], /\{(.*?)\}/g)
        .toArray()
        .filter(key => key !== 'q')
    },
    itemKey() {
